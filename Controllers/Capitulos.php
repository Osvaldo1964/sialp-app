...
class Capitulos extends Controllers
{
    ...
    public function capitulos()
    {
        ...
        $data['page_title'] = "CAPITULOS <small> SIALP - APP </small>";
    ...
    public function getCapitulos()
    {
        if ($_SESSION['permisosMod']['reaPermiso']) {
            ...
            for ($i = 0; $i < count($arrData); $i++) {
                ...
                $btnEdit = '';
                $btnDelete = '';
                if ($arrData[$i]['tipCapitulo'] == 1) {
                    $arrData[$i]['tipCapitulo'] = '<span class="badge badge-success">Ingreso</span>';
                } else {
                    $arrData[$i]['tipCapitulo'] = '<span class="badge badge-danger">Gasto</span>';
                }
                if ($_SESSION['permisosMod']['reaPermiso']) {
                    $btnView = '<button class="btn btn-info btn-sm" onClick="fntViewInfo(' . $arrData[$i]['idCapitulo'] . ')" title="Ver Capitulo"><i class="far fa-eye"></i></button>';
                }
                if ($_SESSION['permisosMod']['updPermiso']) {
                    $btnEdit = '<button class="btn btn-primary btn-sm" onClick="fntEditInfo(this,' . $arrData[$i]['idCapitulo'] . ')" title="Editar"><i class="fas fa-pencil-alt"></i></button>';
                }
                if ($_SESSION['permisosMod']['delPermiso']) {
                    $btnDelete = '<button class="btn btn-danger btn-sm" onClick="fntDelInfo(' . $arrData[$i]['idCapitulo'] . ')" title="Eliminar"><i class="far fa-trash-alt"></i></button>';
                }
    ...
    public function getCapitulo($idcapitulo)
    {
        if ($_SESSION['permisosMod']['reaPermiso']) {
    ...
    public function getSelectCapitulos()
    {
        ...
        if (count($arrData) > 0) {
            for ($i = 0; $i < count($arrData); $i++) {
                $htmlOptions .= '<option value="' . $arrData[$i]['idCapitulo'] . '">' .
                                $arrData[$i]['nomCapitulo'] . '</option>';
            }
        }
        echo $htmlOptions;
        die();

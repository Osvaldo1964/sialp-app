...
class Usos extends Controllers
{
    ...
    public function getUso($idcapitulo)
    {
        if ($_SESSION['permisosMod']['reaPermiso']) {
    ...
    public function getSelectUsos()
    {
        $htmlOptions = "";
        $arrData = $this->model->selectUsos();
        if (count($arrData) > 0) {
            for ($i = 0; $i < count($arrData); $i++) {
                $htmlOptions .= '<option value="' . $arrData[$i]['idTipouso'] . '">' .
                                $arrData[$i]['desTipouso'] . '</option>';
            }
        }
        echo $htmlOptions;
        die();

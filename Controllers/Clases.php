...
class Clases extends Controllers
{
    ...
    public function clases()
    {
        ...
        $data['page_tag']   = "Clases UCAPs";
        $data['page_title'] = "CLASES UCAPs <small> SALP APP </small>";
    ...
    public function getSelectClases(){
        $htmlOptions = "";
        $arrData = $this->model->selectClases();
        if (count($arrData) > 0){
            for ($i=0; $i < count($arrData); $i++){
                if ($arrData[$i]['estClase'] == 1){
                    $htmlOptions .= '<option value="' . $arrData[$i]['idClase'] . '">' .
                                    $arrData[$i]['desClase'] . '</option>';
                }
            }
        }
        echo $htmlOptions;

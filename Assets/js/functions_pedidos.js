...
function fntUpdateInfo(){
    ...
    formUpdatePedido.onsubmit = function(e) {
        ...
        let transaccion;
        //pedidos
        //document.querySelector("#txtTransaccion").value = rowTable.cells[1].textContent;
        //alert(document.querySelector("#txtTransaccion").value);
...
function fntInsertCartera(cliente, pedido, total){
        divLoading.style.display = "flex"; 
        let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
        let ajaxUrl = base_url+'/Pedidos/insertFactura/';
        let formData = new FormData();
        ...
        request.onreadystatechange = function(){
            ...
            if(request.status == 200){
                let objData = JSON.parse(request.responseText);
                if(objData.status){
                    rowTable.cells[1].textContent = objData.factura;
                    factura = objData.factura;
                    //alert('salgo d la funcion con el numero' + factura);
                }else{
                    swal("Error", objData.msg , "error");
                } 
                divLoading.style.display = "none";

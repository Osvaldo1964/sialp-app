...
class GrupossalpModel extends Mysql
{
    ...
    public function insertGruposalp(string $codigo, string $descripcion, float $vidautil, int $estado) {
        $return = 0;
        ...
        if (empty($request)) {
            $query_insert = "INSERT INTO gruposalp (codGruposalp, desGruposalp, vidGruposalp, estGruposalp) VALUES (?,?,?,?)";
            $arrData = array($this->strcodGruposalp, $this->strdesGruposalp, $this->fltvidGruposalp, $this->intestGruposalp);
    ...
    public function updateGruposalp(int $idGruposalp, string $codigo, string $descripcion, float $vidautil, int $estado)
    {
        $this->intidGruposalp   = $idGruposalp;
        $this->strcodGruposalp  = $codigo;
        $this->strdesGruposalp  = $descripcion;
        ...
        $request = $this->select_all($sql);
        if (empty($request)) {
            $sql = "UPDATE gruposalp SET codGruposalp = ?, desGruposalp = ?, vidGruposalp = ?, estGruposalp = ?
